...
static int wfall_width;
static int wfall_height;
static int wfall_left;
static int wfall_top;

static colour16_t *waterfall;
static int waterfall_i = 0;
...
void waterfall_init(int width, int height, int left, int top)
{
    wfall_width = width;
    wfall_height = height;
    wfall_left = left;
    ...
    waterfall = (colour16_t *)malloc(sizeof(colour16_t) * (size_t)(wfall_width * wfall_height));

    // Pre-compute colour values for each of the waterfall intensity levels
    for (int i = 0; i < INTENS_LEVELS; i++)
    {
